...
* MCP is an open standard, not vendor lock-in. 
...
. Navigate to the *MCP Servers* section in the left Navigation Panel to verify MCP support is available.
+
...
. Navigate to the *MCP Servers* panel in the in-chat toolbar.
+
image::module-04-figure-01a.png[Chat toolbar showing MCP Servers icon highlighted with tooltip,align="center",width=500,link=module-04-figure-01a.png^,title="MCP Servers Toolbar Icon"]

. Click *Configure MCP Servers ->* to open the MCP configuration page.
...
. Go back to the *Chat* panel, enable MCP tools:
+
...
+
image::module-04-figure-09a.png[MCP panel showing Enable checkbox checked with time and filesystem servers selected and 16 tools from 2 servers,align="center",width=500,link=module-04-figure-09a.png^,title="MCP Servers Enabled"]
...
. In the *vLLM Server* panel, click the *Stop Server* button to terminate the running vLLM instance.
