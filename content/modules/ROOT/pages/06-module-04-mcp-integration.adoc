...
|**Tools**
|Functions the AI can call (e.g., read_file, get_time)

|**Resources**
|Data sources the AI can access (e.g., files, databases)
...
+
// TODO: Add screenshot
image::mcp-servers-panel.png[MCP Servers panel in sidebar,width=300,title="MCP Servers Panel"]
...
. Click *Add Server* and select the *Time* preset.
+
// TODO: Add screenshot
image::mcp-add-time-server.png[Adding Time MCP server,width=500,title="Add Time Server"]

. The Time server configuration is simple—no additional settings needed. Click *Connect*.
