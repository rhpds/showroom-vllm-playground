...
sudo ln -sf /home/lab-user/.local/bin/uv /usr/local/bin/uv
sudo ln -sf /home/lab-user/.local/bin/uvx /usr/local/bin/uvx

# Verify the symlinks were created
ls -la /usr/local/bin/uvx

# Test with full path (sudo uses restricted PATH)
sudo /usr/local/bin/uvx --version
----
+
NOTE: The symlinks use the full path `/home/lab-user/.local/bin/` because `~` would expand to `/root` when using `sudo`. The vLLM Playground systemd service runs as root, so it needs uvx accessible via `/usr/local/bin`.
+
[source,bash]
----
