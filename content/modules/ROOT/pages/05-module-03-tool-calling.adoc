...
.. Choose a model that supports tool calling (e.g. `Qwen2.5-3B-Instruct`)
+
...
. In vLLM Playground in-chat sidebar, navigate to the *Tools* panel (wrench icon in the toolbar).
+
...
. Create an order status lookup tool (You would need pick up the right info from the definition below (JSON format) to fill into the tools definition form):
+
[source,json]
----
{
...
TIP: In this module, steps 1-3 were demonstrated. Module 4 (MCP Integration) covers steps 4-5 with actual tool execution.
