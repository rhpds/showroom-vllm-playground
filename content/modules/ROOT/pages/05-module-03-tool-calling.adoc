...
WARNING: **Tool calling does NOT mean vLLM executes tools.** When tool calling is enabled, the AI model generates a JSON-formatted function call with arguments—but vLLM itself does not execute any code or call external APIs. Your application is responsible for parsing the tool call, executing the actual function, and returning results to the model. This design ensures security and gives you full control over what actions are taken.

=== Understanding tool calling parsers

Different model families use different formats for tool calling. vLLM Playground supports several parsers:
...
|Model
|`meta-llama/Llama-3.2-3B-Instruct` (or similar Llama 3.x model)

|Enable Tool Calling
|✓ Checked

|Tool Call Parser
|`llama3_json` (or Auto-detect)
|===
+
NOTE: If using a gated model like Llama, ensure you have HuggingFace access configured.

. Click *Start Server* and wait for the model to load.

. Monitor the server logs for tool calling confirmation:
+
[source,bash]
----
...
----
+
Look for messages indicating tool calling is enabled.
...
+
// TODO: Add screenshot
image::tools-panel.png[Tools definition panel,width=600,title="Tools Panel"]
...
+
// TODO: Add screenshot
image::tools-defined.png[Four customer support tools defined,width=600,title="Defined Tools"]
...
+
----
Customer: Hi, I placed an order last week and haven't received any updates. My order number is ORD-78432. Can you check the status?
----
+
...
+
// TODO: Add screenshot
image::tool-call-response.png[AI generating a tool call for order status,width=700,title="Tool Call Generated"]
...
+
----
Customer: I need to update my shipping address. My email is john.smith@email.com
----
+
Expected tool call:
+
...
+
----
Customer: This is frustrating! I've been trying to resolve this billing issue for days. Can someone call me back? My number is 555-123-4567, preferably in the afternoon.
----
+
Expected tool call:
+
...
----
{
  ...
  "arguments": {
    ...
    "issue_summary": "Billing issue unresolved for multiple days"
  }
}
----
...
+
----
Customer: Do you have any wireless headphones under $100?
----
+
Expected tool call:
+
...
+
----
Customer: I want to check on order ORD-99001 and also see if you have the new laptop model in stock.
----
+
...
. Try a different model (Llama 3.1+ has improved tool calling)
...
* link:https://llama.meta.com/docs/model-cards-and-prompt-formats/llama3_1[Llama 3.1 Tool Use^]
