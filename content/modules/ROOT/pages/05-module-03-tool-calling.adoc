...
== Part 1 — Enabling tool calling for intelligent automation
...
+
image::module-03-figure-01.png[Server configuration with tool calling enabled,align="center",width=500,link=module-03-figure-01.png^,title="Enable Tool Calling"]
+
...
+
**Click the Tools icon (🔧) in the toolbar**
+
...
+
image::module-03-figure-02.png[Tools definition panel,align="center",width=600,link=module-03-figure-02.png^,title="Tools Panel"]
...
----
+
image::module-03-figure-03.png[Order status lookup tool definition,align="center",width=600,link=module-03-figure-03.png^,title="Order Status Lookup Tool"]
+
...
→ A: Tool definitions are configured by administrators - the AI can only call tools you explicitly define. In production, you'd add authentication and authorization checks before executing function calls.

**Q: "What if the AI generates invalid function arguments?"**
→ A: Tool definitions include parameter schemas with types and requirements. The AI generates arguments matching this schema. Your backend should still validate before execution.

**Q: "How does this integrate with our existing APIs?"**
→ A: The AI generates standard JSON function calls. Your middleware maps these to your API endpoints - REST, GraphQL, internal services. The pattern works with any backend.

**Q: "What about execution and results?"**
→ A: This module shows function call generation. Module 4 (MCP Integration) demonstrates the full execution cycle with human-in-the-loop approval and results returning to the AI.

== Part 2 — Advanced tool patterns (Optional - 60min demo)
...
* ✓ How tool calling transforms AI from text generation to intelligent automation
* ✓ The business impact of reducing agent handoffs by 60-80%
* ✓ How function definitions enable AI-to-backend system integration
* ✓ Real-world application to customer support workflows (orders, callbacks, account lookup)
* ✓ The pattern for integrating AI with existing enterprise APIs
