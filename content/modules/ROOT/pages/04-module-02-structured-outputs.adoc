...
ACME's engineering team requires predictable, structured outputs that downstream systems can reliably consume. vLLM Server's structured output capabilities, including JSON Schema, Regex, and Grammar, provide exactly this control. This module demonstrates how to constrain LLM outputs to specific formats, ensuring ACME's AI responses are system-ready and consistently parseable.
...
.. Update *Max Model Length* to `8192` (remove the previous `2048` value set for TinyLlama)
.. Click *Clear* in the Chat Interface to clear previous chat history
.. Click *Start Server*
+
...
----
{
 ...
 "priority": "low",
 "summary": "Request for Dark Mode feature in Mobile App",
...
[[part-3]]
== Part 3 - Regex and grammar (optional)
...
----
+
Expected output format: `ACME-BILL-0001` (exact letters/numbers will vary)
+
