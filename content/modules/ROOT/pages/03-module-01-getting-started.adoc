...
[source,text]
----
usage: vllm-playground [-h] [--version] [--host HOST] [--port PORT] [--reload]
...
positional arguments:
  {start,stop,status,pull}
                        Commands
    start               Start the playground (default)
    stop                Stop running playground instance
    status              Check if playground is running
    pull                Pre-download vLLM container images (recommended for
                        first run)
...
options:
  ...
  --port PORT, -p PORT  Port to listen on (default: 7860)
...
+
image::module-01-figure-01.png[vLLM Playground home page ,align="center",width=700,link=module-01-figure-01.png^,title="vLLM Playground Home Page"]
+
--
Observe the Web UI of vllm-playground:

* *Left sidebar* -- Navigation across vLLM projects, management, monitoring, and integrations.
* *Top bar* -- Server status, Podman indicator, dark-mode toggle, and language selector.
* *Server Configuration panel* (left-center) -- Run mode selection and Server Configuration for the vLLM server.
* *Chat Interface panel* (center) -- ChatGPT-style conversation area with message input, token counter, and toolbar.
* *Server Health panel* (right) -- Real-time metrics for KV cache, throughput, and prefix cache.
* *Server Logs panel* (bottom) -- Live log viewer streaming server events.
--

. In the vLLM Playground web interface, configure the first model:
+
.. Navigate to *Server Configuration* section
.. Switch the *Run Mode* to `Container` by clicking on *Container*
...
* *Custom model name*: (Optional) Enter any HuggingFace model ID. Leave blank to use the dropdown selection above.
* *Browse Community Recipes*: (Optional) Explore community-contributed model configurations.
* *HuggingFace Token*: Required only for gated models (Llama 3.1, Llama 3.2, etc.). Leave blank as default.
+
...
.. Double check the *Run Mode* (at the top of *Server Configuration* section), and make sure *Container* mode is selected from previous steps.
+
image::module-01-figure-03.png[Run Mode section with Container mode selected,align="center",width=300,link=module-01-figure-03.png^,title="Run Mode Selection"]
...
* *Metal*: For Mac with M-series chipsets (great for laptop demo)
+
...
+
* *Host*: vLLM Server Host (deafult: 0.0.0.0)
* *Port*: API endpoint port (default: 8000)
* *Tensor Parallel Size*: Number of GPUs for model parallelism (default: 1)
* *GPU Memory (%)*: Fraction of GPU memory to use (default: 90)
* *GPU Status*: A graphic display of accelerator status (read from tool output like `nvidia-smi`)
* *Data Type*: Model precision (auto, float16, bfloat16, float32), leave it as default "Auto".
* *Max Model Length*: Maximum context length (input prompt + output tokens), *set `2048` for TinyLlama model*, optional for other models.
+
NOTE: TinyLlama supports up to 2048 tokens. If you are using GPU mode, enter `2048` in this field -- the default value of 8096 will cause the server to fail to start.
+
...
.. Scroll down, and Click *Start Server*
...
. Point out the *Server Health* panel on the right side of the chat interface. This built-in dashboard displays real-time server metrics:
+
* *KV Cache*: Current key-value cache utilization percentage
* *Reqs*: Active and queued request counts, plus eviction count
* *Throughput*: Prompt processing speed and generation speed (tok/s)
* *Prefix Cache*: Hit rate for prefix caching optimization
* *Open Observability Dashboard*: Link to the full observability view
+
These metrics provide instant visibility into vLLM server performance during inference.
+
image::module-01-figure-08.png[Server Health panel displaying real-time metrics: KV Cache at 0.0 percent, Reqs 0/0 with Evict 0, Throughput showing Prompt 4.6 tok/s and Generation 13.6 tok/s, Prefix Cache Hit Rate 0.0 percent, and Open Observability Dashboard link at bottom,align="center",width=300,link=module-01-figure-08.png^,title="Server Health Panel"]

. Click the *Open Observability Dashboard* link at the bottom of the *Server Health* panel. This opens the full Observability page with detailed performance metrics:
+
* *KV Cache & Memory* -- Cache usage, prefix cache queries and hits, preemptions
* *Request Queue* -- Running and waiting request counts
* *Throughput* -- Average prompt and generation throughput (tok/s)
* *Latency* -- Time to first token, inter-token latency, time per output token, end-to-end request latency
* *Other Metrics* -- Token counts, cache statistics, iteration totals
+
image::module-01-figure-08a.png[Observability Dashboard showing detailed vLLM server metrics including KV Cache and Memory section with cache usage and prefix cache stats and Request Queue with running and waiting requests and Throughput with average prompt 4.6 tok/s and generation 13.6 tok/s and Latency with time to first token 30.0 ms and inter-token latency 17.5 ms and end-to-end request latency 1500.0 ms,align="center",width=700,link=module-01-figure-08a.png^,title="Observability Dashboard"]

. Go back to *vLLM Server* by clicking it from the left navigation bar.
+
image::module-01-figure-08b.png[Left navigation bar showing vLLM Projects with vLLM Server highlighted and GuideLLM and vLLM-Omni below and Management Monitoring and Integrations sections,align="center",width=200,link=module-01-figure-08b.png^,title="Navigation Bar"]
+
--
**What I say:**
...
. Click the gear icon (⚙️) in the toolbar directly below the chat message input area to open the *Chat Settings* panel:
+
* *Temperature*: Controls randomness (lower = focused, higher = creative)
* *Max Tokens*: Maximum response length
* *Show Logprobs*: Token probability heatmap on responses
* *Top-k*: Number of alternative tokens considered per step
+
image::module-01-figure-09.png[Chat Settings panel showing Temperature slider at 0.7 and Max Tokens slider at 256 and Show Logprobs checkbox and Top-k set to 5,align="center",width=500,link=module-01-figure-09.png^,title="Chat Settings"]

. Set a *System Prompt* to define the model's persona and behavior:
+
[source,text]
----
...
----
+
image::module-01-figure-09a.png[System Prompt panel with text area containing You are a helpful customer support assistant for ACME Corporation and Clear button and Templates dropdown,align="center",width=500,link=module-01-figure-09a.png^,title="System Prompt Configuration"]
+
...
+
TIP: `TinyLlama` is a very small model and response quality may not be ideal. For real use cases, use a more capable model such as `Qwen2.5-3B-Instruct` or `Llama-3.1-8B-Instruct` for better results.
+
--
**What they should notice:**
...
. Before moving on, stop the vLLM server by scrolling down to the bottom of the *Server Configuration* panel and clicking *Stop Server*.
+
image::module-01-figure-10.png[Server control buttons showing Save Instance button and Start Server and Stop Server buttons,align="center",width=300,link=module-01-figure-10.png^,title="Stop Server"]
+
TIP: Use *Save Instance* to save the current server configuration for fast loading next time. Saved instances can be managed from the *Instances* page in the left navigation bar.
...
. Check container logs: `sudo podman logs vllm-service`
