...
+
image::module-02-figure-02.png[Model selection showing dropdown and custom model options,width=500,title="Model Selection Options"]
.. Navigate to *Run Mode" section
.. Verify the "Container" mode is selected (default).
.. Navigate to "Compute Mode" section
.. Select "GPU" (NOTE: Both CPU and GPU modes available, but GPU mode will have better performance)
.. Review the default settings (GPU memory utilization, max model length)
.. Click *Start Server*
+
// TODO: Add screenshot
image::server-configuration.png[Server configuration panel with model settings,width=600,title="Configuring vLLM Server"]
