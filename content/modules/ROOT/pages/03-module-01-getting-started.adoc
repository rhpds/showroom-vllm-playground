...
. As verified in Exercise 1, vLLM Playground is already running as a daemon service. Access the Web UI by opening your browser to:
+
...
+
image::module-01-figure-02.png[Model selection showing dropdown and custom model options,width=200,title="Model Selection Options"]
...
+
image::module-01-figure-03.png[Run Mode selection showing Subprocess and Container options,width=200,title="Run Mode Selection"]
...
+
image::module-01-figure-04.png[Compute Mode selection showing CPU and GPU options,width=200,title="Compute Mode Selection"]
...
+
image::module-01-figure-05.png[Command Preview showing the generated vLLM command based on selected parameters,width=200,title="Command Preview"]
...
----
+
Look for a message indicating the server is ready to accept requests.

=== Verify

Run the following to confirm the vLLM server is running:
...
Expected output shows server running:

----
vLLM Playground Status:
  Web UI: Running on port 7860
  vLLM Server: Running (TinyLlama/TinyLlama-1.1B-Chat-v1.0)
  Container: vllm-service (healthy)
----

Also verify via Podman:
...
+
// TODO: Add screenshot
image::chat-interface.png[vLLM Playground chat interface showing conversation area,width=700,title="Chat Interface"]
...
. Test the streaming capability by asking for a longer response:
+
----
Please provide a detailed guide on how to handle common customer complaints, including examples of empathetic responses.
----
+
Watch the response stream in real-time, demonstrating vLLM's efficient inference.
