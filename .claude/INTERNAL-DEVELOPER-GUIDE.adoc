...
**January 12, 2026** - Content Quality and Red Hat Style Enforcement
- **AsciiDoc Formatting**: Added mandatory blank line requirements for all lists
- **Clickable Images**: All images must include `link=self,window=blank`
- **Plagiarism Prevention**: Added Section 7 to SKILL-COMMON-RULES.md for content originality
- **Em Dash Removal**: Added Section 8 to SKILL-COMMON-RULES.md prohibiting em dashes (—)
- Updated verification prompts to actively scan for:
  - List formatting violations
  - Missing clickable image links
  - Plagiarized or copied content
  - Em dash usage (must use commas, periods, or en dashes instead)
- Updated redhat_style_guide_validation.txt with punctuation and plagiarism sections
- Fixed demo template (02-details.adoc) with proper spacing
- Added AsciiDoc Formatting, Plagiarism Prevention, and Em Dash sections to Quality Standards
- Created comprehensive CHANGELOG-2026-01-12.md with 6 major sections
- Updated CLAUDE-SKILLS-GUIDE.adoc with plagiarism and em dash warnings
- Deployed to 3 repositories: genai, vllm, self-service
- Files changed: 11 files, 871 insertions(+), 14 deletions(-)
